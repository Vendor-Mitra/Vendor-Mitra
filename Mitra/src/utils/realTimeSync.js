...
const realTimeSyncApi = {
  ...
  // Emit flash sale update events
  emitFlashSaleUpdate: (action, flashSale) => {
    realTimeSync.emitFlashSaleUpdate(action, flashSale)
  },

  // Subscribe to flash sale update events
  subscribeToFlashSaleUpdates: (callback) => {
    return realTimeSync.subscribe('flash_sale_update', callback)
  },

  // Emit review update events
  emitReviewUpdate: (action, review) => {
    realTimeSync.emitReviewUpdate(action, review)
  },

  // Subscribe to review update events
  subscribeToReviewUpdates: (callback) => {
    return realTimeSync.subscribe('review_update', callback)
  },

  // Emit delivery update events
  emitDeliveryUpdate: (action, delivery) => {
    realTimeSync.emitDeliveryUpdate(action, delivery)
  },

  // Subscribe to delivery update events
  subscribeToDeliveryUpdates: (callback) => {
    return realTimeSync.subscribe('delivery_update', callback)
  },
